...
void spmv_kernel32_sparse_v2l(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, FTYPE *ocsr_ev)
{
	...
	for(l=loc1; l<interm2; l+=4) {
		int e1 = __shfl_sync(0xffffffff,buf, jj,MFACTOR)*sc;
		int e2 = __shfl_sync(0xffffffff,buf, jj+1,MFACTOR)*sc;
		int e3 = __shfl_sync(0xffffffff,buf, jj+2,MFACTOR)*sc;
		int e4 = __shfl_sync(0xffffffff,buf, jj+3,MFACTOR)*sc;
		...
		r3 = __shfl_xor_sync(0xffffffff,r3, 2);
		r4 = __shfl_xor_sync(0xffffffff,r4, 3);
		r1 += r2+r3+r4;
		r1 += __shfl_xor_sync(0xffffffff,r1, 16);
		r1 += __shfl_xor_sync(0xffffffff,r1, 8);
		r1 += __shfl_xor_sync(0xffffffff,r1, 4);
...
__global__
//__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_sparse_v2h(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, FTYPE *ocsr_ev, int *special, int *special2)
{
...
__global__
__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_sparse_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, FTYPE *ocsr_ev)
{
        int idx = (blockIdx.x*SBF)+(threadIdx.x>>5);// + (threadIdx.x>>(LOG_MFACTOR));
        int lane = (threadIdx.x&(MFACTOR-1));
        int offset = lane;
...
__global__
//__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_ssparse(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, FTYPE *ocsr_ev)
{
	int idx = (blockIdx.x*SBF)+(threadIdx.x>>5);// + (threadIdx.x>>(LOG_MFACTOR));
	int lane = (threadIdx.x&(MFACTOR-1));
	int offset = lane;
	...
	for(l=loc1; l<loc2; l++) {
		...
		if(lane == 0) {
			...
			//atomicAdd(&ocsr_ev[l], r*csr_ev[l]); // can be avoided
			ocsr_ev[l] += r*csr_ev[l]; // can be avoided
		}
	}
}
...
__global__
__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_dense_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list, FTYPE *vin, FTYPE *vout, int *baddr, int *saddr, FTYPE *ocsr_ev)
{
        int lane = (threadIdx.x&(MFACTOR-1));
        int offset = lane;
        int loop, i, j;
    ...
    for(i=warp_id;i<BH;i+=(DBSIZE>>LOG_MFACTOR)) {
        ...
        for(l=loc1; l<interm3; l+=2) {
                int e1 = (__shfl_sync(0xffffffff,buf, jj,MFACTOR)&(BW-1));
                int e2 = (__shfl_sync(0xffffffff,buf, jj+1,MFACTOR)&(BW-1));
                ...
                r2 = __shfl_xor_sync(0xffffffff,r2, 1);
                r1 += r2;
	//if(threadIdx.x == 0) printf("b: %d ", r1);
                r1 += __shfl_xor_sync(0xffffffff,r1, 16);
                r1 += __shfl_xor_sync(0xffffffff,r1, 8);
                r1 += __shfl_xor_sync(0xffffffff,r1, 4);
                r1 += __shfl_xor_sync(0xffffffff,r1, 2);
	//if(threadIdx.x == 0) printf("a: %d ", r1);
...
void process()
{
	...
	/////
        ...
        cudaMemcpy(_vout, vout, sizeof(FTYPE)*nr*sc, cudaMemcpyHostToDevice);
...
//fprintf(stderr, "avg : %f, vari : %f, num_dense : %d\n", avg, vari, num_dense);
	...
	if (ne/nc < 6 && vari < 40) { 
		...
		for(int ik=0;ik<ITER;ik++) {
			// kernel fun
			spmv_kernel32_ssparse<<<ss_gridsize, ss_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _ocsr_ev);
		}
		...
		for(int ik=0;ik<ITER;ik++) {
			// kernel fun
			spmv_kernel32_sparse_v2<<<s_gridsize, s_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _ocsr_ev);
			spmv_kernel32_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr, _ocsr_ev);
		}	
		cudaEventRecord(event2,0);
		cudaEventSynchronize(event1);
		cudaEventSynchronize(event2);
		...
		for(int ik=0;ik<ITER;ik++) {
			// kernel fun
			spmv_kernel32_sparse_v2l<<<s_gridsize, s_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _ocsr_ev);
			spmv_kernel32_sparse_v2h<<<s_gridsizeh, s_blocksizeh, 0, stream3>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _ocsr_ev, _special, _special2);
			spmv_kernel32_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt, _mcsr_e, _mcsr_list, _vin, _vout, _baddr, _saddr, _ocsr_ev);
		}	
		cudaEventRecord(event2,0);
		cudaEventSynchronize(event1);
	...
	// FILE *fpo = fopen("SDDMM_GPU_SP.out", "a");
...
#define VALIDATE
			...
			for(i=0;i<nr;i++) {
					for(j=csr_v[i]; j<csr_v[i+1]; j++) {
							int k;
							for(k=0; k<sc; k++) {
									vout_gold[j] += vin[sc*csr_e0[j] + k] * vout[sc*i + k];
							}
							vout_gold[j] *= csr_ev0[j];
					}
			}
			int num_diff=0;
			for(i=0;i<ne;i++) {
					FTYPE p1 = vout_gold[i]; FTYPE p2 = ocsr_ev[i];
