...
void sddmm_GPU(const isratnisa::Matrix S,
    ...
    for (int i = 0; i < S.nnz; ++i) {
        CPU_tot += matrixP[tS.tiled_ind[i]];
        CPU_tot_orig += matrixP[i];
    ...
    for (int i = 0; i < 3; ++i)
        cout << "gp idx " << i << " " << " GPU " << p_ind_temp[i] << " CPU " << matrixP[tS.tiled_ind[i]] << endl;
    ...
    for (int i = 0; i < tS.nnz; ++i) {
        //if(p_ind_temp[i] != 0)
        {
            if (abs(p_ind_temp[i] - matrixP[tS.tiled_ind[i]]) > .000001) {
                diff_tot++;
                if (diff_tot < 5)
                    printf("CPU GPU diff %d:  %f %f %f \n",
                           i,
                           p_ind_temp[i],
                           matrixP[tS.tiled_ind[i]],
                           p_ind_temp[i] - matrixP[tS.tiled_ind[i]]);
            }
        }
    }
...
void sddmm_CPU_COO(const vector<int> row_ind,
                   const vector<int> col_ind,
                   const vector<float> val_ind,
...
void preprocessing(const isratnisa::Matrix S,
    ...
    matrixP = new float[S.nnz]; // output Matrix

    isratnisa::TiledMatrix tiledS(S, tile_sizeX, tile_sizeY);
...
//    make_HTasH(W, W_t, S.n_rows, k); // 可能是按照 COO 格式拷贝, 结果后面没有用到???
    ...
    sddmm_CPU_COO(S.rows, S.cols, S.vals, W, H, matrixP, S);

    /* GPU call */
    sddmm_GPU(S, tiledS, matrixP, W, H);
