...
__global__ void StandKernel(int m,
                            int n,
                            int k,
...
void StandCall(int m,
               ...
               const float* __restrict__ rhs_matrix,
               float* __restrict__ out){
    StandKernel<<<(m + 31) / 32 ,32>>>(m, n, k, row_indices, row_offsets, column_indices, values, lhs_matrix,
                                       rhs_matrix, out);
}
