...
template<typename T>
void sparseMatrix::COO<T>::openTensorCoreMode(const TensorCoreConfig tensorCoreConfig,
                                              MatrixMultiplicationOrder multiplicationOrder) {
    if (tensorCoreMode_) {
        return;
    }
...
template<typename T>
void sparseMatrix::COO<T>::openTensorCoreModeForSampled(TensorCoreConfig tensorCoreConfig) {
    if (tensorCoreMode_) {
        return;
    }
    tensorCoreMode_ = true;
...
template<typename T>
void sparseMatrix::COO<T>::closeTensorCoreMode() {
    if (!tensorCoreMode_) {
        return;
    }
    tensorCoreMode_ = false;
